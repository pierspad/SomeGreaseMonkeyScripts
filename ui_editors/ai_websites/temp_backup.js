...
// @version      3.4
// @description  Indice laterale paginato con sliding window, caricamento stabile, unread solo per nuovi messaggi, compatibile con auto-scroll blocker.
// @author       Tu
...
(function() {
    ...
    function detectActiveBlock() {
        ...
        if (bestIndex !== activeIndex) {
            ...
            // Auto-follow: se non c'è override manuale, tieni il dot attivo nella finestra
            if (!manualOverride) {
            ...
            // Se lo scroll è causato da un click, NON segnare come letti i blocchi intermedi
            if (!clickScrolling) {
                markAsRead(activeIndex);
            }
        }
        return activeIndex;
    }

    // Sposta la finestra per contenere l'indice dato
    ...
    navContainer.addEventListener('click', (e) => {
        // Gestione click frecce paginazione (sliding window)
        ...
        if (currentBlocks[index]) {
            ...
            // Annulla override manuale: la finestra seguirà il dot cliccato
            manualOverride = false;
            clearTimeout(manualOverrideTimeout);
    ...
    function renderDots() {
        ...
        if (needsPagination) {
            ...
            upArrow.innerHTML = '&#9650;';
            upArrow.title = windowStart > 0 ? `${windowStart} pallini sopra` : 'Inizio lista';
            if (windowStart === 0) upArrow.classList.add('is-disabled');
        ...
        if (needsPagination) {
            ...
            downArrow.innerHTML = '&#9660;';
            downArrow.title = endIdx < total ? `${total - endIdx} pallini sotto` : 'Fine lista';
            if (endIdx >= total) downArrow.classList.add('is-disabled');
